...
type GetTimeResponse = {type: 'get-time', data: number | undefined};
...
class Server {
    ...
    recv({id, data: request}: SentRequest): void {
        ...
        if (type === 'get-time') {
            ...
            this.respond<GetObjectResponse>(id, 'get-object', this.world.getObj(data));
        } else if (type === 'get-all-objects') {
            this.respond<GetAllObjectsResponse>(id, 'get-all-objects', this.world.getObjPaths('', true).map(x => {return {path: x, object: this.world.getObj(x)}}));
        } else if (type === 'get-all-object-paths') {
            this.respond<GetAllObjectPathsResponse>(id, 'get-all-object-paths', this.world.getObjPaths('', true));
        } else if (type === 'get-object-count') {
            this.respond<GetObjectCountResponse>(id, 'get-object-count', this.world.getObjPaths('', true).length);
